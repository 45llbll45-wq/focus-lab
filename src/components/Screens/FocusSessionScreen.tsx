...
export const FocusSessionScreen: React.FC<FocusSessionScreenProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col p-5 justify-between space-y-5 text-slate-900">
      ...
      <div className="flex-1 flex flex-col items-center justify-center space-y-5 py-2">
        ...
        <div className="w-full bg-white p-5 rounded-3xl border border-slate-200/90 shadow-sm text-center space-y-2 relative overflow-hidden">
          <span className="inline-block text-[11px] font-bold text-indigo-700 bg-indigo-50 px-2.5 py-0.5 rounded-full border border-indigo-200">
            الخطوة الحالية
          </span>
        ...
        <div className="relative flex flex-col items-center justify-center">
          ...
          <div className="relative w-52 h-52 flex items-center justify-center">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
              ...
              <circle
                ...
                className={`transition-all duration-1000 ease-linear stroke-current ${
                  timeLeft === 0 ? 'text-amber-500' : 'text-indigo-600'
                }`}
                strokeWidth="6"
                strokeDasharray="264"
            ...
            <div className="absolute flex flex-col items-center justify-center text-center">
              <span className={`text-4xl font-extrabold tracking-tight font-mono ${
                timeLeft === 0 ? 'text-amber-600 animate-pulse' : 'text-slate-900'
              }`}>
                {formatTime(timeLeft)}
              </span>
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            <button
              ...
              className="px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold flex items-center gap-1 transition-all cursor-pointer border border-slate-200"
              title="إضافة 5 دقائق"
            >
            ...
            <button
              onClick={resetTimer}
              className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-400 hover:text-slate-700 transition-all cursor-pointer border border-slate-200"
