...
export const ContentGeneratorScreen: React.FC<ContentGeneratorScreenProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col p-5 space-y-4 text-[#1e1b24]">
      {/* 1. Header */}
      <div className="space-y-1">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs text-[#3e2f59] hover:bg-[#c6ed58]/35 hover:text-[#1e1b24] flex items-center gap-1 cursor-pointer font-bold bg-[#dbb0cf]/25 px-2.5 py-1 rounded-xl border border-[#dbb0cf]/40 transition-colors"
            title="إعادة الصياغة"
          >
      ...
      <div className="space-y-1.5">
        <label className="text-xs font-bold text-[#1e1b24] block">نوع القالب:</label>
        <div className="grid grid-cols-4 gap-1.5 bg-[#dbb0cf]/25 p-1 rounded-2xl border border-[#dbb0cf]/40">
          {formats.map((fmt) => {
            ...
            return (
              <button
                ...
                className={`py-2 px-1 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1 cursor-pointer ${
                  active
                    ? 'bg-[#3e2f59] text-[#c6ed58] shadow-xs'
                    : 'text-[#6b6475] hover:text-[#1e1b24]'
                }`}
              >
                <span>{fmt.label}</span>
              </button>
      ...
      <div className="space-y-3 flex-1 flex flex-col">
        ...
        <div className="space-y-1">
          <label className="text-xs font-bold text-[#1e1b24] flex items-center gap-1">
            ...
            <Edit3 className="w-3 h-3 text-[#878191]" />
          </label>
          <input
            type="text"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            className="w-full p-2.5 bg-white text-xs sm:text-sm font-bold rounded-xl border border-[#ece7de] focus:border-[#3e2f59] outline-none shadow-xs text-[#1e1b24]"
          />
        </div>
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            <label className="text-xs font-bold text-[#1e1b24] flex items-center gap-1">
              ...
              <span className="bg-[#c6ed58] text-[#1e1b24] px-2 py-0.5 rounded text-[10px] font-black border border-[#b8e244]">
                عالي التفاعل ⚡
              </span>
            </label>
            <button
              onClick={() => handleCopy(hook, 'hook')}
              className="text-[11px] text-[#3e2f59] hover:text-[#1e1b24] flex items-center gap-1 cursor-pointer font-bold bg-[#dbb0cf]/25 hover:bg-[#dbb0cf]/45 px-2 py-0.5 rounded-lg border border-[#dbb0cf]/40 transition-colors"
            >
              {copiedField === 'hook' ? <Check className="w-3 h-3 text-[#3e2f59]" /> : <Copy className="w-3 h-3" />}
          ...
          <textarea
            ...
            rows={2}
            className="w-full p-2.5 bg-[#c6ed58]/15 border border-[#c6ed58]/40 text-xs sm:text-sm font-bold rounded-xl focus:border-[#3e2f59] outline-none resize-none shadow-xs text-[#1e1b24]"
          />
        </div>
        ...
        <div className="space-y-1 flex-1 flex flex-col">
          <div className="flex items-center justify-between">
            <label className="text-xs font-bold text-[#1e1b24] flex items-center gap-1">
              ...
              <span className="text-[10px] text-[#878191] font-normal">(قابلة للتعديل)</span>
            </label>
            <button
              onClick={() => handleCopy(body, 'body')}
              className="text-[11px] text-[#3e2f59] hover:text-[#1e1b24] flex items-center gap-1 cursor-pointer font-bold bg-[#dbb0cf]/25 hover:bg-[#dbb0cf]/45 px-2 py-0.5 rounded-lg border border-[#dbb0cf]/40 transition-colors"
            >
              {copiedField === 'body' ? <Check className="w-3 h-3 text-[#3e2f59]" /> : <Copy className="w-3 h-3" />}
          ...
          <textarea
            ...
            rows={6}
            className="w-full p-3 bg-white border border-[#ece7de] text-xs sm:text-sm font-normal rounded-2xl focus:border-[#3e2f59] focus:ring-2 focus:ring-[#dbb0cf]/25 outline-none resize-none shadow-xs text-[#1e1b24] leading-relaxed flex-1 min-h-[140px]"
          />
        </div>
      </div>
      ...
      <div className="pt-2 sticky bottom-4 z-20">
        <Button
          ...
          size="lg"
          fullWidth
          icon={<Save className="w-5 h-5 text-[#c6ed58]" />}
