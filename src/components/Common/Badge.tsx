...
export const Badge: React.FC<BadgeProps> = ({
  ...
  const variantStyles = {
    ...
    primary: 'bg-indigo-50 text-[#4F46E5] border border-indigo-200/80 font-bold',
    accent: 'bg-violet-50 text-violet-700 border border-violet-200/80 font-bold',
...
export const CategoryBadge: React.FC<{ category: TaskCategory }> = ({ category }) => {
  const configs: Record<TaskCategory, { label: string; icon: string; className: string }> = {
    learn: { label: 'أتعلم', icon: '🧠', className: 'bg-indigo-50 text-[#4F46E5] border-indigo-200/80' },
    build: { label: 'أبني', icon: '⚡', className: 'bg-violet-50 text-violet-700 border-violet-200/80' },
    content: { label: 'محتوى', icon: '✍️', className: 'bg-sky-50 text-sky-700 border-sky-200/80' },
...
export const EnergyBadge: React.FC<{ energy: EnergyLevel }> = ({ energy }) => {
  const configs: Record<EnergyLevel, { label: string; icon: string; className: string }> = {
    ...
    medium: { label: 'طاقة متوسطة', icon: '⚡', className: 'bg-indigo-50 text-indigo-700 border-indigo-200/80' },
    high: { label: 'طاقة عالية', icon: '🔥', className: 'bg-rose-50 text-rose-700 border-rose-200/80' }
  };
...
export const ContentFormatBadge: React.FC<{ format: ContentFormat }> = ({ format }) => {
  const configs: Record<ContentFormat, { label: string; className: string }> = {
    ...
    carousel: { label: 'Carousel 📑', className: 'bg-indigo-50 text-indigo-700 border-indigo-200/80' },
    post: { label: 'Post 📝', className: 'bg-sky-50 text-sky-700 border-sky-200/80' },
